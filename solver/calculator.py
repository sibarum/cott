...
class FullScreenViewer:
    ...
    def _draw_grid(self):
        ...
        tick_step = _nice_tick_step(self.bounds)
        grid_color = '#1a1a2e'
        label_color = '#444466'
        ...
        while p <= p_max:
            ...
            self.canvas.create_text(sx + 4, ch - 4, text=_tick_label(round(p, 10)),
        ...
        while q <= q_max:
            ...
            self.canvas.create_text(4, sy - 4, text=_tick_label(round(q, 10)),
                                    font=font, fill=label_color, anchor='sw', tags='grid')
            q += tick_step
    ...
    def _do_render(self, quality='high'):
        ...
        # Snapshot current view for this render
    ...
    def _compute_phase_rgb(self, res, cw, ch, center_p, center_q, bounds):
        ...
        app = self.app
        expr_text = app.entry_var.get().strip()
        if not expr_text:
            return None
        ...
        from traction import traction_simplify
    ...
    def _compute_fractal_rgb(self, res, cw, ch, cancel, center_p, center_q, bounds):
        app = self.app
        expr_str, escape, max_iter = _parse_fractal_args(app._fractal_raw_text)
